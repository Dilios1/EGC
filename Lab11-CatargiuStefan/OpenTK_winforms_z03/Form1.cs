...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        //FLUTURE
        private float butterflyPositionX = 0.0f;
        private float butterflyPositionY = 0.0f;
        private float butterflyPositionZ = 0.0f;
        private bool statusButterflyLoaded = false;
        private float wingAngle = 0.0f;
        private float wingSpeed = 5.0f;
        private float animationSpeed = 2.0f;

        //TRANSPARENT OBJ
        private bool statusTranspObj = false;
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            if(statusButterflyLoaded==true)
            {
                DrawButterfly (butterflyPositionY);
            }

            if(statusTranspObj==true)
            {
        ...
        private void DrawObjectTransparent()
        {
        ...
        //Fluture.
        private void DrawButterfly(float butterflyPositionY)
        {
            GL.PushMatrix();
            GL.Translate(0.0f, butterflyPositionY, 0.0f); // Aplică poziția fluturelui
            GL.Scale(3.0, 3.0, 1.0);
            ...
            // Head antenas (linii)
            Color headColor = GetRandomColor();
            GL.Begin(PrimitiveType.Lines);
            GL.Color3(headColor);
            GL.Vertex2(-2, 9.4);
            GL.Vertex2(-2, 11.4);
            ...
            // Random Color
            Color wingColor1 = GetRandomColor();
            Color wingColor2 = GetRandomColor();
            ...
            // Left wing
            GL.PushMatrix();
            GL.Translate(-12.0, 2.5, 0.0);
            GL.Rotate(-45 + wingAngle, 0.0, 0.0, 1.0); // Ajustează unghiul de rotație
            GL.Scale(3.0, 3.0, 1.0);
            GL.Begin(PrimitiveType.Triangles);
            GL.Color3(wingColor1);
            GL.Vertex2(0, 0);
            GL.Color3(wingColor1);
            GL.Vertex2(5, 5);
            GL.Color3(wingColor1);
        ...
        private void btnLoadButterfly_Click(object sender, EventArgs e)
        {
            
            statusButterflyLoaded = true;

            
            GlControl1.Invalidate();
        }

        private void btnResetBtf_Click(object sender, EventArgs e)
        {
            statusButterflyLoaded=false;
            butterflyPositionY = 0.0f;
            wingAngle = 0.0f;
            GlControl1.Invalidate();

        }

        private void btnAnimButterfly_Click(object sender, EventArgs e)
        {
            butterflyPositionY += animationSpeed;

            
            wingAngle = (float)Math.Sin(DateTime.Now.Ticks / 10000000.0 * wingSpeed) * 45.0f; 

            // Redesenare fereastră
            GlControl1.Invalidate();
        }
        ...
        private void buttonTranspObj_Click(object sender, EventArgs e)
        {
            statusTranspObj = true;
